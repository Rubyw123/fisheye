...
def update_blob_tracker(blob, blob_id, frame):
    ...
    return (blob_id, blob)
def add_simplified_id(blob_id):
    s_ids.update({blob_id,(len(s_ids)+1)})

def remove_simplified_id(blob_id):
    s_ids.pop(blob_id)

def get_simplified_id(blob_id):
    return s_ids.get(blob_id)
